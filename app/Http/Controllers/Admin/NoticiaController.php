...
class NoticiaController extends Controller
{
    ...
    public function update(Request $request, $id){
        ...
        if($noticia->save()){
            return redirect()->route("admin.noticias.edit", $noticia->id)->with("exito", "Se actualizó la noticia");
        }
        return redirect()->route("admin.noticias.edit", $noticia->id)->with("error", "No se pudo actualizar la noticia");
    }
    ...
    public function destroy($id){
        $noticia = Noticia::find($id);
        if($noticia->delete()){
            return redirect()->route("admin.noticias.index")->with("exito", "Se eliminó correctamente");
        }
        return redirect()->route("admin.noticias.index")->with("error", "No se pudo elimiminar la noticia");
    }
